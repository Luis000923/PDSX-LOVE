...
$user = require_login();
$pdo  = db();
$templates = $pdo->query('SELECT id, slug, name, description, thumbnail, kind, category, is_premium, price_usd, price_coins FROM templates WHERE is_active = 1 ORDER BY is_premium, price_usd, id')->fetchAll();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        ...
        $res = Sites::create($user, $tpl, $data);
        if (isset($res['slug'])) {
...
<form method="post" id="crear" class="grid gap-8 lg:grid-cols-[1fr_380px] lg:items-start">
  <?= csrf_field() ?>
        ...
        <?php foreach ($templates as $t): $locked = $isLocked($t); $cost = Access::coinCost($user, $t); ?>
          <div class="relative flex flex-col rounded-2xl border border-rose-100 bg-white overflow-hidden text-sm transition has-[:checked]:border-rose-500 has-[:checked]:bg-rose-50 has-[:checked]:ring-2 has-[:checked]:ring-rose-300 has-[:focus-visible]:ring-2 has-[:focus-visible]:ring-rose-600 <?= $locked ? '' : 'hover:border-rose-300' ?>">
            <label class="block <?= $locked ? 'opacity-60' : 'cursor-pointer' ?>">
              <input class="sr-only" type="radio" name="template_id" value="<?= (int) $t['id'] ?>" data-name="<?= e($t['name']) ?>" data-cost="<?= (int) $cost ?>" data-slug="<?= e($t['slug']) ?>" <?= (int) $t['id'] === $chosen && !$locked ? 'checked' : '' ?> <?= $locked ? 'disabled' : '' ?>>
              ...
              <span class="block p-3 pb-1">
                <span class="flex items-center gap-1.5 font-semibold"><?php if ($locked): ?><img src="<?= e(url('assets/img/paginas/ico-lock.svg')) ?>" alt="" width="16" height="16" loading="lazy"><?php endif; ?><?= e($t['name']) ?></span>
                <span class="mt-1 block text-xs font-semibold <?= $locked ? 'text-amber-800' : 'text-emerald-800' ?>"><?= $locked ? (Access::isPurchasable($t) ? 'Bloqueada · $' . e(wompi_format_usd(Access::priceInCents($t))) . ' USD' : 'Bloqueada · Solo Premium') : ($cost > 0 ? $cost . ' monedas' : 'Gratis') ?></span>
              </span>
            </label>
...
<?php endif; ?>
<?php if (array_filter($templates, $isLocked) || $atLimit): ?>
