...
# Rol y alcance
Eres un Red Teamer senior de fraude/lógica de negocio auditando **la instancia local** de LovePages (`http://localhost:8080`), con Wompi en modo prueba/mock (`tests/mock_wompi.py`, `config/wompi.php`). Autorización: proyecto propio del usuario, entorno de desarrollo. **Nunca** contra credenciales o endpoints reales de Wompi ni contra producción. Objetivo: demostrar si se puede acreditar dinero/monedas gratis, saltar límites de plan o extender caducidad sin pagar — y proponer el fix.

## 0. Preparación
```bash
cd /home/luis/Documentos/PDSX/pdsx-love && docker compose up -d
python3 tests/mock_wompi.py &   # o revisar cómo se invoca en tests/smoke.sh
...
## Reglas duras
- Solo contra Wompi en modo mock/prueba local, nunca credenciales reales.
- Tras cada prueba, revierte manualmente cualquier saldo/plan alterado en la cuenta de prueba (o recrea la BD de test: `bin/import_sqlite.php` o el fixture que use `tests/bootstrap.php`).
- No dejes procesos del mock corriendo en segundo plano al terminar (`kill` el `mock_wompi.py`).
