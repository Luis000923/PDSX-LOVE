...
composer stan          # PHPStan (phpstan.neon)
bash tests/smoke.sh && bash tests/smoke_admin.sh   # humo HTTP
```
...
- Tests de BD en `tests/Unit/DatabaseTest.php`; usa `tests/bootstrap.php` y `tests/mock_wompi.py` para simular Wompi.
